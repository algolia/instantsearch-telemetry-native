  ...
  static func random() -> Self {
    TelemetrySchema.with {
      $0.components = Array(TelemetryComponentType.allCases.map { type in
        TelemetryComponent.with { component in
          component.type = type
          component.isConnector = Bool.random()
          component.isDeclarative = Bool.random()
          component.parameters = Array(TelemetryComponentParams.allCases.shuffled().prefix((1..<10).randomElement()!))
        }
      }
        .shuffled()
  ...
  func testEncodeDecode() throws {
    let schema = TelemetrySchema.with {
      $0.components = TelemetryComponentType.allCases.map { type in
        TelemetryComponent.with { component in
          component.type = type
          component.isConnector = true
          component.parameters = Array(TelemetryComponentParams.allCases.shuffled()[..<5])
        }
      }
    }
  ...
  func testStringDecoding() throws {
    ...
    for (version, strings) in versionedStrings {
      for (index, string) in strings.enumerated() {
        do {
          _ = try TelemetrySchema(gzippedBase64String: string)
        } catch let error {
